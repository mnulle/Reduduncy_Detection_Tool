...
// for computing i*tˆ8 mod P(t)
void multiT8(usInt *words, usInt *P, short wCount);
// for computing i*tˆ(k-8) mod P(t)
...
f4 = fP[wCount - 1] & 0xff;
for( j = wCount - 1; j > 0; j--) {
fP[j] = fP[j-1] ˆ TA[f1][j] ˆ TB[f2][j] ˆ TC[f3][j] ˆ TD[f4][j];
}
fP[0] = W ˆ TA[f1][0] ˆ TB[f2][0] ˆ TC[f3][0] ˆ TD[f4][0];
}
double getArandom() {
