...
int main( int argc, char * argv[] ) {
	...
	free(fName);
	inName = argc > 3 ? strdup(argv[3]) : strdup("infile.txt");
	...
	while ( fgets(url, 1600, inFile) != NULL ) {
		...
		int counter = 0;
		while(counter + window < len) {		
			// output the result
			for ( j = wCount - 1; j >= 0; j-- ) {
				fprintf(outFile, "%X ", fP[j]);
			}
			fprintf(outFile, "\n");
			fp1shift(fP, P, wCount, url[counter+window], url[counter], tP);
			counter++;
		}
		for ( j = wCount - 1; j >= 0; j-- ) {
			fprintf(outFile, "%X ", fP[j]);
...
void fp1shift(usInt *fP, usInt *P, short wCount, char s, char x, usInt *tP) {
	...
	for ( i = 0; i < 8; i++ ) {
		W = s & 0x80; // retrieve the MSB of s
		Y = x & 0x80; // retrieve the MSB of x
		s = s << 1; // shift for next round
		x = x << 1;
		if(Y) {
			for( j = wCount - 1; j >= 0; j--) 
