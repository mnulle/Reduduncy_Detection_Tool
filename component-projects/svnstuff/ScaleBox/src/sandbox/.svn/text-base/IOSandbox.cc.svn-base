...
IOSandboxModule::IOSandboxModule () : IOModule () {
	m_sSandboxID = "Sandbox";
}


IOSandboxModule::~IOSandboxModule () {
	
}

char IOSandboxModule::initialize () {
...
bool IOSandboxModule::startModule () {
...
bool	IOSandboxModule::extractExtendedDOM (NodeDOM * pNode) {
	int			j;
	NodeDOM	*	pChild;
	...
	for(j=0; j<pNode->getNumChildren(); j++) {
		if(pNode->getChild(j)->getTag() == "sandboxID") {
			m_sSandboxID = pNode->getChild(j)->getData();		
		}	
	}
...
NodeDOM * IOSandboxModule::getStats () {
	return NULL;
}
