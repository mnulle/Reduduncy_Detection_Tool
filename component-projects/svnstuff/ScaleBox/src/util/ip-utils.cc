...
string IPv4toString (char * pData) {
	...
	sprintf(szTemp, "%3d.%3d.%3d.%3d", (unsigned char) pData[0], (unsigned char) pData[1], (unsigned char) pData[2], (unsigned char) pData[3]);
	sResult = szTemp;
...
//	cout << "CTD (pC2): " << pComp2->tv_sec << "." << pComp2->tv_usec << endl;
	...
	if(bT1) {		
		if(nDiffSec == 0) {	
			nDiffUS = pComp1->tv_usec - pComp2->tv_usec;
		} else {
		...
		if(nDiffSec == 0) {	
			nDiffUS = pComp2->tv_usec - pComp1->tv_usec;
		} else {
...
unsigned short	purgeTCPOptions (char * pData, int nLength) {
	...
	// Force the TCP header size to 20 bytes
